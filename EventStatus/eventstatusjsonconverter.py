...
import json
args = sys.argv
path = 'EventStatus/data/Spanish/annotated_texts'
outname = "eventstatus_spa"
if len(args)> 1 :
    path = args[1]
if len(args)> 2:
    outname = args[2]
...
def jsonconvert(path,filelist):
    print("Converting files in %s" %path)
    ...
    for bothdoc in bothdocs:
        ...
        bothjson.append(d2)
    print("Number of files converted for cuno and both :")
    print(len(cunodocs))
    print(len(bothdocs))
